...
namespace com.passioncitychurch.familyregistration
{
    ...
    /// </summary>
    [ToolboxData("<{0}:PassionPreRegistrationStudents runat=server></{0}:PassionPreRegistrationStudents>")]
    public class PassionPreRegistrationStudents : PreRegistrationChildren
    {
        ...
        /// </value>
        public List<PassionStudentRow> PassionStudentRows
        {
            get
            {
                var rows = new List<PassionStudentRow>();
                foreach ( Control control in Controls )
                {
                    if ( control is PassionStudentRow)
                    {
                        var newGroupMemberRow = control as PassionStudentRow;
        ...
        protected override void CreateChildControls()
        {
            ...
            spanAddFilter.InnerHtml = " Add Student";
        ...
        public override void RenderControl( HtmlTextWriter writer )
        {
            if ( this.Visible )
            {
                ...
                foreach ( Control control in Controls )
                {
                    ...
                    {
                        i++;
                        var childRow = control as PassionStudentRow;
                        childRow.Caption = $"{i.ToOrdinalWords().Titleize()} Student";
        ...
        public void ClearRows()
        {
            ...
            {
                if ( Controls[i] is PassionStudentRow)
                {
                    Controls.RemoveAt( i );
                }
            }
